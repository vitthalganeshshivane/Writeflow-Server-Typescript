...
const cleanText = (value) => String(value || "")
    .replace(/\s+/g, " ")
    .trim();
const parseTitleList = (value) => value
    .split(/\r?\n/)
    ...
    .filter(Boolean)
    .slice(0, 3);
const normalizeGhostCompletion = (prefix, output) => {
...
const limitWords = (text, maxWords = 6) => text.split(" ").slice(0, maxWords).join(" ");
const isTitleAlreadyGood = (input) => {
    const words = input.trim().split(/\s+/).length;
    return words >= 8;
};
// ---------- AI CALL ----------
async function providerChat(provider, messages, temperature = 0.4, modelOverride) {
...
// ---------- MAIN ----------
async function generateTitles({ provider, model, mode, input, description = "", tags = [], }) {
    const contextLines = [
        `User text: ${input}`,
...
FINAL OUTPUT:`;
    ...
    const content = await providerChat(provider, [
        {
            role: "system",
            content: "You generate short, clean, high-quality blog titles. Never over-generate.",
        },
        { role: "user", content: prompt },
    ], mode === "suggest" ? 0.7 : 0.4, model);
    if (mode === "suggest") {
        const titles = parseTitleList(content);
        return { titles };
    }
    if (mode === "ghost") {
        if (isTitleAlreadyGood(input))
            return { completion: "" };
